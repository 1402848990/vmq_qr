...
pjysdk = PJYSDK(app_key='d56e8ljdqusoofc2bpqg',
                app_secret='bj2tK9YgiJ7M3a6B5J3BdZuQq7Jc9pTt')
pjysdk.debug = False
...
def save_auth_code(auth_code: str):
    """保存授权码到本地配置文件"""
    try:
        config = {}
        if os.path.exists(CONFIG_FILE):
...
class QRCodeApp(ttk.Window):
    def __init__(self):
        ...
        if not is_access:
            messagebox.showerror("错误", f"❌ {is_access}无权限，请联系✈️:maotai8866")
            return
    ...
    def build_ui(self):
        ...
        # 授权码输入区域
        auth_label = ttk.Label(top_frame, text="授权码:", font=("微软雅黑", 9))
        auth_label.pack(side=LEFT, padx=5)

        self.auth_code_var = tk.StringVar()
        self.auth_code_var.set(load_auth_code())  # 自动回填已保存的授权码

        auth_entry = ttk.Entry(
            top_frame, textvariable=self.auth_code_var, width=25, font=("微软雅黑", 9))
        auth_entry.pack(side=LEFT, padx=5)

        # 保存授权码的函数
        def save_auth():
            auth_code = self.auth_code_var.get().strip()
            if save_auth_code(auth_code):
                self.gui_log(f"✅ 授权码已保存")
            else:
                self.gui_log(f"❌ 授权码保存失败")

        # 绑定失去焦点事件，自动保存
        auth_entry.bind("<FocusOut>", lambda e: save_auth())
        # 绑定回车键，保存授权码
        auth_entry.bind("<Return>", lambda e: save_auth())

        # 保存按钮
        btn_save_auth = ttk.Button(
            top_frame, text="💾 保存", command=save_auth, bootstyle=INFO)
        btn_save_auth.pack(side=LEFT, padx=5)
