...
async def save_qrcode_if_new(url: str, qr_content: str, group_name: str, sender_name: str, log_func):
    ...
    async with db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            await conn.commit()
            if cur.rowcount > 0:
                log_func(f"💾 [{datetime.now().strftime('%H:%M:%S')}] 新二维码已存库（MD5: {qr_md5[:8]}...）")
                return True
            else:
                log_func(f"⏭️ [{datetime.now().strftime('%H:%M:%S')}] 二维码内容已存在（MD5: {qr_md5[:8]}...）")
                return False
...
async def fetch_latest_images(limit=10):
    async with db_pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute("""
                SELECT url, qr_content, group_name, sender_name, created_at 
                FROM qrcode_images 
                ...
                if code == 200:
                    ...
                    if self.ser == 3 or not self.is_connected:
                        ...
                        if self.send_task is None or self.send_task.done():
                            ...
                            self.log(f"⏰ [{datetime.now().strftime('%H:%M:%S')}] {SEND_INTERVAL}秒周期发送任务已启动")

            except json.JSONDecodeError:
                self.log(f"❌ [{datetime.now().strftime('%H:%M:%S')}] JSON解析失败：{msg_content[:200]}")
            except Exception as e:
                self.log(f"❌ [{datetime.now().strftime('%H:%M:%S')}] 消息处理异常：{e}")
            ...
            try:
                ...
                if self.qun_lists:
                    ...
                    for idx, v in enumerate(self.qun_lists):
                        ...
                        self.log(f"----发送请求----群名：{v['name']} | SER：{self.ser} | 群ID：{v['id']}")
...
class QRCodeApp(ttk.Window):
    def __init__(self):
        ...
        # 启动日志监听器（独立线程）
        self.log_listener = threading.Thread(target=self._log_consumer, daemon=True)
    ...
    def build_ui(self):
        ...
        self.btn_start = ttk.Button(top_frame, text="▶ 启动代理", command=self.start_proxy, bootstyle=SUCCESS)
        self.btn_start.pack(side=LEFT, padx=5)

        self.btn_stop = ttk.Button(top_frame, text="⏹ 停止代理", command=self.stop_proxy, bootstyle=DANGER, state=DISABLED)
        self.btn_stop.pack(side=LEFT, padx=5)

        self.btn_refresh = ttk.Button(top_frame, text="🔄 刷新图片", command=self.load_images_from_db)
        self.btn_refresh.pack(side=LEFT, padx=5)

        # === 中部图片展示区域（带滚动）===
        mid_frame = ttk.LabelFrame(self, text="最近识别的二维码图片", padding=10)
        mid_frame.pack(fill=BOTH, expand=YES, padx=10, pady=5)

        canvas = tk.Canvas(mid_frame)
        scrollbar = tkttk.Scrollbar(mid_frame, orient="vertical", command=canvas.yview)
        ...
        log_scroll = tkttk.Scrollbar(log_frame, orient="vertical", command=self.log_text.yview)
    ...
    def _log_consumer(self):
        ...
        while True:
            try:
                ...
                if msg == "__STOP__":
                    break
                self.after(0, lambda m=msg: self.log_text.insert(tk.END, m + "\n"))
    ...
    def start_proxy(self):
        ...
        self.proxy_thread = threading.Thread(target=self._run_proxy_in_thread, daemon=True)
    ...
    async def _start_proxy_async(self):
        ...
        self.gui_log("="*60)
        self.gui_log(f"✅ mitmproxy 8.0 WSS周期发送代理已启动")
        self.gui_log(f"📌 监听地址：http://{PROXY_HOST}:{PROXY_PORT}")
        self.gui_log(f"📌 发送间隔：{SEND_INTERVAL}秒")
        self.gui_log(f"📌 二维码图片将异步存入MySQL（自动去重）")
        self.gui_log("="*60)
    ...
    def load_images_from_db(self):
        ...
        def _fetch_and_update():
            async def _inner():
                try:
                    records = await fetch_latest_images(limit=12)
                    self.after(0, self._update_image_display, records)
                except Exception as e:
                    self.after(0, self.gui_log, f"❌ 加载图片失败: {e}")

            asyncio.run_coroutine_threadsafe(_inner(), self.loop).result()
    ...
    def _update_image_display(self, records):
        ...
        for rec in records:
            try:
                ...
                # 注意：这里为了简单使用同步 requests；生产环境建议用 asyncio + aiohttp
                import requests
                img_data = requests.get(rec['url'], timeout=5).content
